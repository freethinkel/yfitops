...
impl Login5Manager {
    ...
    /// yfitops: accept a token obtained elsewhere. The login5 request only
    /// works when the stored credentials match the client id, which is never
    /// the case for a token taken from the web player's cookie — but the token
    /// itself is perfectly good, so it is handed over directly.
    pub fn set_auth_token(&self, token: Token) {
        self.lock(|inner| inner.auth_token = Some(token));
    }

    pub async fn auth_token(&self) -> Result<Token, Error> {
        // yfitops: cache first, so a token set from outside makes the request
        // below unnecessary — the original order rejected it before looking

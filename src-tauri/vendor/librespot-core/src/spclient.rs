...
impl SpClient {
    ...
    /// yfitops: accept a client token fetched elsewhere. The request below
    /// describes a native client, which the web player's client id refuses —
    /// and its own JSON form has no equivalent in this crate's protocol.
    pub fn set_client_token(&self, token: Token) {
        self.lock(|inner| inner.client_token = Some(token));
    }
